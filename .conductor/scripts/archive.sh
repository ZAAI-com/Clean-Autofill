...
# workspace is archived. Warns about work about to go out of view, then
# reclaims this workspace's build state.
#
# No "set -e" and always exits 0: a cleanup hiccup must never block an archive.
set -uo pipefail
...
# Hard-coded allow list. Both are gitignored and fully regenerated by setup, and
# each is re-checked to be a real directory resolving inside this workspace, so
# nothing else can ever be removed (including a gitignored .env or credential).
echo ''
